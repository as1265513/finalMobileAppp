...
const OpenGooogleMaps =async(lat,lng)=>{
  const scheme = Platform.select({ ios: 'maps:0,0?q=', android: 'geo:0,0?q=' });
  const latLng = `${lat},${lng}`;
  const label = 'Resturent';
  ...
  const onCenter = () => {
    ...
    console.log('location ', curLoc.latitude);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomCard}>
        ...
        <TouchableOpacity onPress={()=>OpenGooogleMaps(location?.latitude,location?.longitude)} style={styles.inpuStyle}>
